...
import { isEligibleTrendingNote, trimNoteContentForCache } from "./noteContent.js";
import {
  TRENDING_RELAY,
  WINE_TRENDING_API,
  WINE_TRENDING_LIMIT,
...
import { fetchVertexProfilePubkeys } from "./vertexProfiles.js";
import {
...
export { fetchVertexProfilePubkeys } from "./vertexProfiles.js";
export {
  countHashtagTags,
  ...
  BANNED_LINK_HOSTS,
  BANNED_HASHTAGS,
  MAX_CACHED_NOTE_CONTENT_CHARS,
  trimNoteContentForCache,
} from "./noteContent.js";
...
/** Drop event signatures from API/cache payloads (UI unused; saves Runtime Cache write units). Keeps tags. */
function stripNoteSig(note) {
  if (!note || typeof note !== "object" || note.sig == null) return note;
  const { sig: _sig, ...rest } = note;
  return rest;
}

/** Strip sig + cap content length before Runtime Cache / CDN JSON. */
function prepareNoteForCache(note) {
  return stripNoteSig(trimNoteContentForCache(note));
}

async function toServerTrendingFeed(notes, engagementById, hours) {
  const withContent = notes.filter(isEligibleTrendingNote);
  const [engagement, spamIds, vertexProfilePubkeys, zapCorrections] =
    await Promise.all([
      ...
      fetchZapObserverCorrections(hours),
    ]);
  const correctedEngagement = applyZapObserverCorrections(
    withContent,
    engagement,
    zapCorrections
  );
  ...
  // Rank + engagement floor on full content (link/hashtag demotion), then
  // shrink for cache writes — trim before the floor would miss demotion in
  // the truncated tail.
  const ranked = rankTrendingNotes(visible, correctedEngagement, {
  ...
  return {
    notes: limited.notes.map(prepareNoteForCache),
 ...
 * Does not apply the Fayan filter (client setting).
 */
