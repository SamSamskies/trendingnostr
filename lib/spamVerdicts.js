...
/**
 * Remove event ids from the LLM spam store (false-positive correction).
 *
 * @param {string[]} ids
 * @returns {Promise<{ removed: number, total: number }>}
 */
export async function removeSpamVerdicts(ids) {
  const store = await readSpamVerdictStore();
  let removed = 0;
  for (const raw of ids) {
    if (!isEventId(raw)) continue;
    const id = raw.toLowerCase();
    if (store.verdicts[id]) {
      delete store.verdicts[id];
      removed += 1;
    }
  }
  if (removed === 0) {
    return { removed: 0, total: Object.keys(store.verdicts).length };
  }
  ...
  return { removed, total: Object.keys(store.verdicts).length };
}

/**
 * Wipe the LLM spam store.
 * @returns {Promise<{ removed: number }>}
 */
export async function clearSpamVerdicts() {
  const store = await readSpamVerdictStore();
  const removed = Object.keys(store.verdicts).length;
  const next = { updatedAt: Math.floor(Date.now() / 1000), verdicts: {} };
  await runtimeCache.set(RUNTIME_CACHE_KEY, next, {
    ttl: SPAM_VERDICT_TTL_SEC,
    tags: ["trending", "spam-verdicts"],
    name: "llm-spam-verdicts",
  });
